...
from timing import run_bat_with_timing as bat_timing
...
def run_analyse_bat_maxflow(csv_path, source='s', sink='t', show_plot=True):
    ...
    print(df)
    best_val, best_paths, best_edges, history = bat_timing(df, source=source, sink=sink)
    ...
    # Vẽ hội tụ
    draw_history(history, embed=True, ax=ax1,
